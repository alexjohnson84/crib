...
class TestDeck(unittest.TestCase):
    ...
    def test_draw_actions(self):
        ...
        self.assertTrue(isinstance(dc, str))
        # Cut Cards
        cc = cd.cut_card()
        self.assertEqual(len(cd.deck), 50)
        self.assertTrue(isinstance(cc, str))
    ...
    def test_deal(self):
        ...
        self.assertTrue(isinstance(hands[0][0], str))
...
class TestGamePlay(unittest.TestCase):
    ...
    def test_turn(self):
        ...
        self.assertTrue(isinstance(new_status['faceup'], str))
...
class TestSpecialCases(unittest.TestCase):
    """
    Tests set up for special cases that were not covered in previous samples.
    Future cases will be added here if bugs are found
    """
...
class TestPegScoring(unittest.TestCase):
    """
    Run through outlined pegging scenario and validate that against results
    from the API
    """
    with open('gameplay/test_files/test_hands.txt', 'r') as th:
    ...
    def test_peg_scoring(self):
        for ex_type, test_set in self.test_hands.iteritems():
            for example in test_set.values():
                ...
                self.assertEqual(
                    cps.score, example['score'], "%s != %s, example: %s for hist %s" %
                    (cps.score, example['score'], ex_type, example['hist']))
                if 'count' in example:
                    self.assertEqual(
                        cps.count, example['count'], "%s != %s, example: %s for hist %s" %
                        (cps.count, example['count'], ex_type, example['hist']))
